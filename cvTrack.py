...
def setRoi(event, x, y, flags, params):
    ...
    # print('\nStrting drawingRect: ', drawingRect)
        ...
        if pTL != pBR:
            ...
            if dy:
                dy = pxBlock - dy
            roiRect.append((pTL, (pBR[0] + dx, pBR[1] + dy)))
...
def drawRoi(frame, pos):
    img = frame.copy() # Copy image to not draw in over the original frame
    cv2.putText(img, str(pos + 1), (0, img.shape[0] - 30), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 0, 0), 4)
    if roiRect or drawingRect:
        rcs = roiRect + [drawingRect] if drawingRect else roiRect
        rt = max(1, int(w / w0) + 1)
        for rc in rcs:
...
cv2.imshow(windowName, img)
cv2.createTrackbar('slider', windowName, 0, total - 1, on_change)
